...
module if_id(
	...
	always @ (posedge clk) begin
		...
		// end else if(stall[1] == `NoStop) begin//取指不停
		end else if(scnt_o != 3'd0 || store_baseram)begin
			if(stall)begin 
				id_pc <= id_pc;
				id_inst <= id_inst;
			end else if(store_baseram ||sdelaysignal ||sdelaysignal_bubble)begin
				id_pc <= id_pc;
				id_inst <= 32'h34000000;
			end else if(sdelaysignal_use ) begin
				id_pc <= id_pc_delay;
		...
		end else if(cnt_o != 3'd0 || pre_load_baseram ||bingo_delay)begin
			...
			if(stall || bingo_delay2)begin
				id_pc <= id_pc;
				id_inst <= id_inst;
			end else if((cnt_o == 3'd3 && !bingo) || bingo_delay )begin //&& !(exe_store && delaysignal_use))||bingo
				id_pc <= id_pc_delay;
				id_inst <= id_inst_delay;
			end	else if(pre_load_baseram  ||delaysignal ||delaysignal_bubble || delaysignal_use ||bingo)begin//||bingo
				id_pc <= id_pc;
				id_inst <= 32'h34000000;
			end else begin
				id_pc <= if_pc;
	...
	always @ (posedge clk)begin
		...
		end else if((pre_load_baseram && !stall) || (store_baseram && !stall))begin
